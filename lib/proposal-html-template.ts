...
function offerAndOptionsPageHtml(data: ProposalDocumentData): string {
  const { services, financial, totals, reference, options } = data;
...
function financialSummaryAndSignaturePageHtml(data: ProposalDocumentData): string {
...
export function buildProposalDocumentHtml(data: ProposalDocumentData): string {
  return `<div class="proposal-document">${coverPageHtml(data)}${missionPageHtml(
    data
  )}${offerAndOptionsPageHtml(data)}${financialSummaryAndSignaturePageHtml(data)}</div>`;
}
